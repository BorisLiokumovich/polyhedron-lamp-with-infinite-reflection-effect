void buttonTick() {
  ...
  if (is_working) {
    ...
    //if ((touch.hasClicks()) && (touch.getClicks() == 4)) {}  // если было пятикратное нажатие на кнопку, то производим сохранение параметров // && (touch.hasClicks())
    if (touch.isHolded()) {
      max_bright_changing_dir = !max_bright_changing_dir;
      numHold = 1;
    }
    //if (touch.isSingle() and touch.isHolded()) {
    // FastLED.clear();
    //numHold = 2;
    ...
    if (touch.isStep()) {
      ...
      switch (numHold) {
        case 1:
          ...
          bright = max_bright;
          for (int i = 0; i < 3; i++) {
            leds[i].setHSV(max_bright, 255, 255);
          }
          break;
        case 2:
