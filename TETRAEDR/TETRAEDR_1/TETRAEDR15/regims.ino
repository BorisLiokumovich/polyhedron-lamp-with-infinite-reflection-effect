...
void simm_cometa() {
  for (int i = 0; i < 6; i++) {                                                                                                                    //режимы из вершин
    for (int j = 0; j < EDGE_LEDS / 2; j++) {                                                                                                      //для каждого ребра
      ...
      perlin_leds(i * EDGE_LEDS + j, perlin_sat_min - j * 10, perlin_sat_max - j * 10, perlin_brig_min, constrain(perlin_brig_max - int(1.5 * j * j),0,255));            //цвет как обычно //насыщенность уменьшаем
      perlin_leds((i + 1) * EDGE_LEDS - j - 1, perlin_sat_min - j * 10, perlin_sat_max - j * 10, perlin_brig_min, constrain(perlin_brig_max - int(1.5 * j * j),0,255));  //яркость уменьшаем по параболе
      perlin_counter += perlin_counter_shift;                                                                                                      //двигаем счетчик, чтобы цвета менялись от точки к точке
    }
    perlin_counter = perlin_counter_zero;  //забываем про изменения счетчика
  }
