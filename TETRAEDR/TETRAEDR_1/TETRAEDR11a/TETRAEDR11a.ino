// ===== НАСТРОЙКИ =====
#define EDGE_LEDS 10  // кол-во диодов на ребре ТЕТРАЭДРА
#define LED_DI 2      // пин подключения
#define BRIGHT 250    // яркость
#define THETIME 10     //время между смено режимов
#define NUM_EDGE 24 //кол во граней

const int NUM_LEDS = (EDGE_LEDS * NUM_EDGE);  //кол во светодиодов
...
void simm_perlin() {
  for (int i = 0; i < NUM_EDGE; i++) {                                                           //для каждой из сторон
...
void simm_cometa() {
  for (int i = 0; i < NUM_EDGE; i++) {      //режимы из вершин
    for (int j = 0; j < EDGE_LEDS / 2; j++) {             //для каждого ребра
      ...
      perlin_leds(i * EDGE_LEDS + j, perlin_sat_min - j * (perlin_sat_min / EDGE_LEDS), perlin_sat_max - j * (perlin_sat_min / EDGE_LEDS), perlin_brig_min, perlin_brig_max - (perlin_brig_max / ((EDGE_LEDS / 2 + 10) ^ 2))* j * j); //цвет как обычно //насыщенность уменьшаем
      perlin_leds((i + 1) * EDGE_LEDS - j - 1, perlin_sat_min - j * (perlin_sat_min / EDGE_LEDS), perlin_sat_max - j * (perlin_sat_min / EDGE_LEDS), perlin_brig_min, perlin_brig_max - (perlin_brig_max / ((EDGE_LEDS / 2 + 10) ^ 2))* j * j); //яркость уменьшаем по параболе
      perlin_counter += perlin_counter_shift;  //двигаем счетчик, чтобы цвета менялись от точки к точке
    }
    perlin_counter = perlin_counter_zero;                                      //забываем про изменения счетчика
  }
...
void cometa() {                            //комета нормальная
  if (head - cometa_lenght == head_end) {  //если комета полностью зашла
    head = random(0, NUM_EDGE) * EDGE_LEDS;       //рандомим светлейшую точку
    head_end = head + EDGE_LEDS;           //и конец
    head_begin = head - 1;                 //и начало
    shift_time = random(40, 100);           //и время
...
void cometaReverced() {                       //комета ненормальная
  if (headr + cometa_lenghtr == head_endr) {  //выполняем действия, ровно аналогичные вышеописанным
    headr = random(1, NUM_EDGE) * EDGE_LEDS;
    head_endr = headr - EDGE_LEDS - 1;
    head_beginr = headr;
    shift_timer = random(40, 100);
...
void fading() {         //темним
  if (bright <= 0) {    //если дотемнили
    ...
    regim = 5;//random(9);                      //рандомим режим
    switch (regim) {                        //и соответственно его выбираем
      case 0:                               //кометы
        //реверснутая
        headr = random(1, NUM_EDGE) * EDGE_LEDS;   //выбираем ярчайшую точку
        ...
        //адекватная
        head = random(0, NUM_EDGE) * EDGE_LEDS;    //ну тут все тоже самое
  ...
  if (fadingdir) {                        //если темним
    if (millis() - prewfading > 5) {     //(темнить надо по времени)
    ...
    if (millis() - prewfading > 5) {     //если светлим
